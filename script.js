...
let API_key = "ed0f73f71b5cf5389144363e2e85c510";
...
async function addCity() {
    ...
    // Setting new debounce timer 
    const inputValueTrimmed = inputValue.value.trim("");

    if(!inputValueTrimmed){
        alert("Enter City Name or Country")
    }
      ...
      debounceTimer = setTimeout(async () => {
        try {
          let getData = await fetch(
            `https://api.openweathermap.org/data/2.5/weather?q=${inputValueTrimmed}&appid=${API_key}&units=metric`
          );
  
          if (!getData.ok) {
            throw new Error(`Failed to fetch data. Status: ${getData.status}`);
          }
